    ...
    <main className="relative max-w-5xl mx-auto px-6 py-12">
      <div className="flex items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="mt-1.5 text-sm text-zinc-500">{rows.length} total signups</p>
        </div>
        <a
          href={`data:text/csv;charset=utf-8,${encodeURIComponent(
            ["email,name,company,source,created_at",
             ...rows.map((r) =>
               [r.email, r.name ?? "", r.company ?? "", r.source, r.created_at]
          ...
          <tbody className="divide-y divide-white/[0.05]">
            ...
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-white/[0.02]">
                ...
                <td className="px-4 py-2.5 text-zinc-500 text-xs">{row.source}</td>
